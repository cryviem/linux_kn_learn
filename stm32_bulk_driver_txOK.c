...
#define STM32_IN_EP_MAX_SIZE				64

#define STM32_BULK_OUT_EP				0x01
#define STM32_OUT_EP_MAX_SIZE				64
...
struct usb_stm32 {
	...
	char *			stm32_Rx_Bulk_Buffer;		
	char *			stm32_Tx_Bulk_Buffer;		
	size_t			stm32_Rx_Packet_Size;		
	size_t			stm32_Tx_Packet_Size;		
	__u8			stm32_BULK_IN_EPAddr;		
	__u8			stm32_BULK_OUT_EPAddr;						
...
{	
	...
	kfree (dev->stm32_Rx_Bulk_Buffer);
	kfree (dev->stm32_Tx_Bulk_Buffer);
	kfree (dev);
}
...
static void stm32_read_callback(struct urb *urb)
{
	...
	copy_to_user(pUSB_stm32_dev->stm32_Rx_UsrSpc_Buf, pUSB_stm32_dev->stm32_Rx_Bulk_Buffer, pUSB_stm32_dev->stm32_Rx_Actual_Size);
...
static ssize_t stm32_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	usb_fill_bulk_urb(pl_Urb, pUSB_stm32_dev->stm32_udev,
			  usb_rcvbulkpipe(pUSB_stm32_dev->stm32_udev, pUSB_stm32_dev->stm32_BULK_IN_EPAddr),
			  pUSB_stm32_dev->stm32_Rx_Bulk_Buffer, count, stm32_read_callback, pUSB_stm32_dev);	
	...
	printk(KERN_INFO "stm32_read: rx urb submit successfully");
	//return l_RetVal;
...
static ssize_t stm32_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(pUSB_stm32_dev->stm32_Tx_Bulk_Buffer, user_buffer, count)) 
	{
		l_RetVal = -EFAULT;
		goto error0;
	}
	...
	/* initialize the urb properly */
	usb_fill_bulk_urb(pl_Urb, pUSB_stm32_dev->stm32_udev,
			  usb_sndbulkpipe(pUSB_stm32_dev->stm32_udev, pUSB_stm32_dev->stm32_BULK_OUT_EPAddr),
			  pUSB_stm32_dev->stm32_Tx_Bulk_Buffer, count, stm32_write_callback, pUSB_stm32_dev);	
...
static int stm32_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	printk(KERN_INFO "stm32_probe: 0 - start probe function.\n");

	/* allocate usb_stm32 struct*/
	printk(KERN_INFO "stm32_probe: 1 - allocate usb_stm32 struct.\n");
	pUSB_stm32_dev = kmalloc(sizeof(struct usb_stm32), GFP_KERNEL);
	if (pUSB_stm32_dev == NULL) {
		printk(KERN_INFO "stm32_probe: 1.1 - allocate usb_stm32 struct FAIL.\n");
		goto error;
	}
	printk(KERN_INFO "stm32_probe: 2 - allocate usb_stm32 struct DONE.\n");
	...
	for (l_ForLoopIndex_UB = 0; l_ForLoopIndex_UB < pInterface_Descriptor->desc.bNumEndpoints; l_ForLoopIndex_UB++) 
	{
		...
		/* debug */
		printk(KERN_INFO "debug 0: loop index: %d\n", l_ForLoopIndex_UB);
		if (!pUSB_stm32_dev->stm32_BULK_IN_EPAddr)
		{
			printk(KERN_INFO "IN_EP 0: pUSB_stm32_dev->stm32_BULK_IN_EPAddr == 0 : PASSED");
		   	if ((pEndpoint_Descriptor->bEndpointAddress & USB_DIR_IN) == USB_DIR_IN)
			{
				printk(KERN_INFO "IN_EP 1: Check EP is IN direction : PASSED");
		   		if ((pEndpoint_Descriptor->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK)== USB_ENDPOINT_XFER_BULK)
				{
					printk(KERN_INFO "IN_EP 2: Check EP is BULK type : PASSED");
					/* USB IN EP found */
					printk(KERN_INFO "stm32_probe: 3 - EP In found at loop index: %d\n", l_ForLoopIndex_UB);
					l_MaxPacketSize_UW = pEndpoint_Descriptor->wMaxPacketSize;
					pUSB_stm32_dev->stm32_Rx_Packet_Size = l_MaxPacketSize_UW;
					pUSB_stm32_dev->stm32_BULK_IN_EPAddr = pEndpoint_Descriptor->bEndpointAddress;
					printk(KERN_INFO "stm32_probe: 3 - EP address: %d\n", pUSB_stm32_dev->stm32_BULK_IN_EPAddr);
					pUSB_stm32_dev->stm32_Rx_Bulk_Buffer = kmalloc(l_MaxPacketSize_UW, GFP_KERNEL);
					if (pUSB_stm32_dev->stm32_Rx_Bulk_Buffer == NULL) {
						printk(KERN_INFO "stm32_probe: 3.1 - Rx Bulk Buffer FAIL allocating.\n");
						goto error;
					}
				}
			}
		...
		if (!pUSB_stm32_dev->stm32_BULK_OUT_EPAddr)
		{
			printk(KERN_INFO "OUT_EP 0: pUSB_stm32_dev->stm32_BULK_OUT_EPAddr == 0 : PASSED");
			if ((pEndpoint_Descriptor->bEndpointAddress & USB_DIR_IN) == 0)
			{
				printk(KERN_INFO "OUT_EP 1: Check EP is OUT direction : PASSED");
		   		if ((pEndpoint_Descriptor->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK)== USB_ENDPOINT_XFER_BULK)
				{
					printk(KERN_INFO "OUT_EP 2: Check EP is BULK type : PASSED");
					/* USB OUT EP found */
					printk(KERN_INFO "stm32_probe: 3 - EP Out found at loop index: %d\n", l_ForLoopIndex_UB);
					l_MaxPacketSize_UW = pEndpoint_Descriptor->wMaxPacketSize;
					pUSB_stm32_dev->stm32_Tx_Packet_Size = l_MaxPacketSize_UW;
					pUSB_stm32_dev->stm32_BULK_OUT_EPAddr = pEndpoint_Descriptor->bEndpointAddress;
					printk(KERN_INFO "stm32_probe: 3 - EP address: %d\n", pUSB_stm32_dev->stm32_BULK_OUT_EPAddr);
					pUSB_stm32_dev->stm32_Tx_Bulk_Buffer = kmalloc(l_MaxPacketSize_UW, GFP_KERNEL);
					if (pUSB_stm32_dev->stm32_Tx_Bulk_Buffer == NULL) {
						printk(KERN_INFO "stm32_probe: 3.1 - Tx Bulk Buffer FAIL allocating.\n");
					goto error;
					}
				}
			}
	...
	if ((pUSB_stm32_dev->stm32_BULK_OUT_EPAddr == 0) || (pUSB_stm32_dev->stm32_BULK_IN_EPAddr == 0))
	{
		printk(KERN_INFO "stm32_probe: 4 - Check EPs FAIL.\n");
		goto error;
	}
	printk(KERN_INFO "stm32_probe: 4 - Check EPs OK.\n");
	...
	if (retval)
	{
		printk(KERN_INFO "stm32_probe: 5 - register FAIL.\n");
		usb_set_intfdata(interface, NULL);
		goto error;
	}
	else
	{
		printk(KERN_INFO "stm32_probe: 5 - register OK.\n - Minor obtained: %d\n", interface->minor);
	}
